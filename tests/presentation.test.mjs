...
test('more product work uses a balanced four-column gallery with the requested final projects', async () => {
  const server = await createServer({ server: { middlewareMode: true, ws: false }, appType: 'custom', logLevel: 'error' })
  try {
    const { render } = await server.ssrLoadModule('/src/entry-server.jsx')
    const html = render('/')
    ...
    assert.equal((supporting.match(/data-project-cover="gallery"/g) || []).length, 8)
    for (const slug of ['learning-mobility', 'samsung-podcast', 'b4q4-widgets']) assert.match(supporting, new RegExp(`/work/${slug}/`))
    for (const removedSlug of ['handsign', 'wood-chip']) assert.doesNotMatch(supporting, new RegExp(`/work/${removedSlug}/`))
    for (const image of ['Portfolio_Mobility.webp', 'Portfolio_Podcast.webp', 'Portfolio_B4Q4.webp']) assert.match(supporting, new RegExp(image.replace('.', '\\.')))
    assert.equal((supporting.match(/data-project-slug=/g) || []).length, 8)
...
test('about page presents a branded toolkit, compact profile links and school identities', async () => {
  ...
  try {
    ...
    assert.equal((html.match(/data-tech-tool="true"/g) || []).length, 24)
    for (const tool of ['Figma', 'Photoshop', 'Illustrator', 'Miro', 'Framer', 'React', 'JavaScript', 'TypeScript', 'HTML / CSS', 'Python', 'Node.js', 'C++', 'Git', 'FFmpeg', 'Blender', 'Unity', 'Unreal Engine', 'Claude', 'Gemini', 'OpenAI API', 'Agentic AI', 'MCP', 'Multi-agent', 'Prompt Design']) assert.match(html, new RegExp(`>${tool.replace('+', '\\+')}<`))
    ...
    assert.match(currentCss, /\.techTools\s*{[^}]*grid-template-columns:\s*repeat\(12,\s*minmax\(0,\s*1fr\)\)/s)
    assert.match(currentCss, /\.techTools\s*{[^}]*gap:\s*clamp\(/s)
    assert.match(currentCss, /\.techTool\s*{[^}]*border:\s*0/s)
...
test('homepage about includes the complete toolkit and a compact school list', async () => {
  ...
  try {
    ...
    assert.equal((about.match(/data-home-tech-tool="true"/g) || []).length, 24)
    for (const tool of ['Figma', 'Photoshop', 'Illustrator', 'Miro', 'Framer', 'React', 'JavaScript', 'TypeScript', 'HTML / CSS', 'Python', 'Node.js', 'C++', 'Git', 'FFmpeg', 'Blender', 'Unity', 'Unreal Engine', 'Claude', 'Gemini', 'OpenAI API', 'Agentic AI', 'MCP', 'Multi-agent', 'Prompt Design']) assert.match(about, new RegExp(`>${tool.replace('+', '\\+')}<`))
