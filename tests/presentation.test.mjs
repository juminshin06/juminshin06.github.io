...
test('about page presents a branded toolkit, compact profile links and school identities', async () => {
  ...
  try {
    ...
    assert.equal((html.match(/data-tech-tool="true"/g) || []).length, 20)
    for (const tool of ['Figma', 'Photoshop', 'Illustrator', 'Miro', 'Framer', 'React', 'JavaScript', 'TypeScript', 'HTML / CSS', 'Python', 'Node.js', 'C++', 'Git', 'FFmpeg', 'Blender', 'Unity', 'Unreal Engine', 'Claude', 'Gemini', 'OpenAI API']) assert.match(html, new RegExp(`>${tool.replace('+', '\\+')}<`))
    ...
    assert.match(currentCss, /\.techTools\s*{[^}]*grid-template-columns:\s*repeat\(10,\s*minmax\(0,\s*1fr\)\)/s)
...
test('homepage about includes the complete toolkit and a compact school list', async () => {
  ...
  try {
    ...
    assert.equal((about.match(/data-home-tech-tool="true"/g) || []).length, 20)
    for (const tool of ['Figma', 'Photoshop', 'Illustrator', 'Miro', 'Framer', 'React', 'JavaScript', 'TypeScript', 'HTML / CSS', 'Python', 'Node.js', 'C++', 'Git', 'FFmpeg', 'Blender', 'Unity', 'Unreal Engine', 'Claude', 'Gemini', 'OpenAI API']) assert.match(about, new RegExp(`>${tool.replace('+', '\\+')}<`))
